...
internal class ConfirmUserInvitationCommandHandler(
    IEnumerable<IValidator<ConfirmUserInvitationCommand>> validators, ILogger<ConfirmUserInvitationCommandHandler> logger,
    IRepository<User> repository, IIntegrationEventPublisher integrationEventPublisher, IAuthTokenProvider authTokenProvider,
    IClock clock) : CommandHandler<ConfirmUserInvitationCommand>(validators, logger)
{
    protected override async Task<CommandResult> HandleInternal(ConfirmUserInvitationCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            await integrationEventPublisher.PublishAsync(new UserInvitationConfirmedIntegrationEvent(user.Id, now), cancellationToken);
