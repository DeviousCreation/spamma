...
internal class GetChaosMonkeyAddressesByGridParamsQueryProcessor(IDbContextFactory<SpammaDataContext> dbContextFactory)
    : IQueryProcessor<GetChaosMonkeyAddressesByGridParamsQuery, GetChaosMonkeyAddressesByGridParamsQueryResult>
{
    public async Task<QueryResult<GetChaosMonkeyAddressesByGridParamsQueryResult>> Handle(
        GetChaosMonkeyAddressesByGridParamsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (data == null)
        {
            return QueryResult<GetChaosMonkeyAddressesByGridParamsQueryResult>.Failed();
        }

        var count = await countFuture.ExecuteAsync(cancellationToken);

        return QueryResult<GetChaosMonkeyAddressesByGridParamsQueryResult>.Succeeded(new GetChaosMonkeyAddressesByGridParamsQueryResult(
            data.Select(x => new GetChaosMonkeyAddressesByGridParamsQueryResult.Item(
                x.ChoasMonkeyAddressId,
                x.EmailAddress,
                x.ChaosMonkeyType,
                x.SubdomainId)).ToList(), count));
    }
}